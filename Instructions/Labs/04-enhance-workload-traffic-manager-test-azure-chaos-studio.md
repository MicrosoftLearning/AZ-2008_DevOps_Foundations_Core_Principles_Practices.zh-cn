...
在本练习中，你将实现一个复原配置，该配置使用 Azure 流量管理器在两个不同的 Azure 区域中的两个 .NET Web 应用实例之间分发请求。
...
1. 在显示 Azure 门户的 Web 浏览器选项卡中，在页面顶部的搜索文本框中输入 **`Traffic Manager profiles`**，然后在结果列表中选择“**流量管理器配置文件**”。
1. 在“**负载均衡服务\|流量管理器**”页上，选择“**+ 创建**”。
1. 在“**创建流量管理器配置文件**”页上，执行以下操作：
   ...
   - 在“**资源组**”下拉列表下方选择“**新建**”链接，在“**名称**”文本框中，输入 **`rg-devops-foundations`**，然后选择“**确定**”。
...
1. 在“**负载均衡服务\|流量管理器**”页上，根据需要选择“**刷新**”，然后选择“**devopsfoundationstmprofile**”。
...
1. 在“**添加终结点**”窗格中，执行以下操作：
   ...
   - 在“名称”文本框中，输入 **`DevOps Foundations web app - East US`**。
   - 确保选中“**启用终结点**”复选框。
...
1. 在“**添加终结点**”窗格中，执行以下操作：
   ...
   - 确保“**Azure 终结点**”显示在“**类型**”下拉列表中。
   - 在“名称”文本框中，输入 **`DevOps Foundations web app - West Europe`**。
   - 确保选中“**启用终结点**”复选框。
   - 在“**目标资源类型**”下拉列表中，选择“应用服务”。
...
1. 在“**试验设计器**”选项卡上，执行以下操作：
   ...
   - 在“**步骤**”文本框中，输入 **`Step 1: Failover an App Service web app`**。
   - 在“**分支**”文本框中，输入 **`Branch 1: Emulate an App Service failure`**。
...
1. 在 Azure 门户中，在页面顶部的搜索文本框中，输入 **`App Services`**，并在结果列表中选择“**应用服务**”。
...
1. 在 Web 应用页上的“**Essential**”部分中，请注意，Web 应用的状态将列为“**已停止**”。
   ...
   > **备注：** 你可能需要选择“**刷新**”工具栏条目来更新 Web 应用的状态。
...
1. 在显示 Azure 门户的 Web 浏览器选项卡中，在页面顶部的搜索文本框中输入 **`Resource groups`**，然后在结果列表中选择“**资源组**”。
