...
- 需要至少有“参与者级访问权限”的 Azure 订阅。 如果没有 Azure 订阅，可以注册[免费试用版](https://azure.microsoft.com/free)。
...
在本练习中，你将使用 CI/CD 实现 IaC，并使用 GitHub Actions 和 Bicep 模板实现 Azure 应用服务 Web 应用。
...
1. 在 Azure 门户中页面顶部的搜索文本框中，输入“资源组”，然后在结果列表中选择“资源组”********。
1. 在“资源组”页面上，选择“创建”********。
...
1. 复制命令的整个 JSON 格式输出并记录它。 稍后需要它。 输出的格式应类似于以下文本：
...
1. 在显示 Azure 门户的 Web 浏览器窗口中，在页面顶部的搜索文本框中，输入“应用服务”，并在结果列表中选择“应用服务”********。
